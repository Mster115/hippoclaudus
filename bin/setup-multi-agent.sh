#!/usr/bin/env bash
# hippoclaudus: multi-agent skill installer
...
#   - Codex CLI      : symlink to ~/.codex/skills/hippoclaudus
#   - OpenCode       : symlink to ~/.opencode/skills/hippoclaudus
#   - Gemini CLI     : symlink to ~/.gemini/skills/hippoclaudus
...
if [ ! -d "$SKILLS_DIR" ]; then
  echo "ERROR: $SKILLS_DIR does not exist. Are you running this from the hippoclaudus repo?"
  exit 1
fi
...
echo "hippoclaudus: multi-agent skill installer"
echo "Repo: $REPO_ROOT"
echo

# Codex CLI
link_if_missing "$SKILLS_DIR" "$HOME/.codex/skills/hippoclaudus" "Codex CLI"

# OpenCode
link_if_missing "$SKILLS_DIR" "$HOME/.opencode/skills/hippoclaudus" "OpenCode"

# Gemini CLI
link_if_missing "$SKILLS_DIR" "$HOME/.gemini/skills/hippoclaudus" "Gemini CLI"
...
echo "  - Codex CLI:   codex --list-skills | grep hippoclaudus"
